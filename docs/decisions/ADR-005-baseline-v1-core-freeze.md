...
| 4 ago 2026 | `crearOrchestrator()` — zona de wiring (`modules/orchestrator.js`, registro de `ActionRunner`) | Se registró la acción `ejecutar_motor_ingenieria` (mismo lugar y mismo patrón que `crear_ticket_soporte`, precedente de ADR-012) — el handler real vive en `modules/cotizaciones.js` (no Core); no se tocó la clase `Orchestrator`, `_finalizarWorkflow`, `_ejecutarAcciones` ni `procesarMensaje` | Fase 2 del módulo Ingeniería y Cotización: el nodo final del workflow "Cotización directa" (migración 090) necesita disparar el motor de predimensionamiento solar contra datos reales | (a) ramificar el resultado (bloqueado/completo) dentro de `_finalizarWorkflow` para elegir el mensaje de cierre — descartada explícitamente: el propio ADR-005 nombra ese caso como el que sí requeriría tocar Orchestrator; en vez de eso el handler manda su propio mensaje de seguimiento proactivo, fuera del Core |
